		...
		function(m,oa){
			...
			function Yb(a,b,e){
				...
				function f(){
					...
					y(b.defaultView);m(window).resize(na);
					t()||g()
					}
				function g(){
					setTimeout(
				...
				X.gotoDate=p;
				X.incrementDate=L;
				X.formatDate=function(k,D){//添加月份和星期和时间
					var num=Oa(k,D,b),rt,long=num.length;
					if(num.charAt(l)==="星"){
						if(long>3)
							rt="<img src=\"/TaxPersonalReporting1.0/fullcalendar/demos/images\\"+num.substring(0,3)+".jpg\"/>"+num.substring(3);
						else
							rt="<img src=\"/TaxPersonalReporting1.0/fullcalendar/demos/images\\"+num+".jpg\"/>";
					}else{
						rt=num;
					}
					return rt;
					};
				X.formatDates=function(k,D,Z){//获取时间
					return ib(k,D,Z,b)
					};
				X.getDate=c;
				X.getView=z;
								...
								m.each(this.split(","),function(ra,sa){
									if(sa=="title"){
										...
										ga=null
											...
											var ha;if(a[sa])ha=a[sa];
											else if(Ja[sa])
												ha=function(){
												ma.removeClass(q+"-state-hover");
												a.changeView(sa)};
												if(ha){
													ra=b.theme?jb(b.buttonIcons,sa):
														null;
													var da=jb(b.buttonText,sa),
													ma=m("<span class='fc-button fc-button-"
															+sa+" "
															+q+
													...
													if(ma){ma.click(
															function(){
																ma.hasClass(q+"-state-disabled")||ha()}).mousedown(
																		function(){
																			ma.not("."+q+"-state-active").not("."+q+"-state-disabled").addClass(q+"-state-down")}).mouseup(
																					function(){
																						ma.removeClass(q+"-state-down")}).hover(
																								function(){
																									ma.not("."+q+"-state-active").not("."+q+"-state-disabled").addClass(q+"-state-hover")
																									},
																									function(){
																										ma.removeClass(q+"-state-hover").removeClass(q+"-state-down")}).appendTo(fa);
			...
			function $b(a,b){
				...
				function na(c){
					var z=c.source||{},
					H=Ta(z.ignoreTimezone,
							a.ignoreTimezone);
					c._id=c._id||(c.id===oa?"_fc"+bc++:c.id+"");
					if(c.date){
						if(!c.start)c.start=c.date;delete c.date
						}
			...
			function Oa(a,b,e){//添加星期
				return ib(a,null,b,e)
				}
			function ib(a,b,e,d){//返回星期和特殊标记
				d=d||Ya;var f=a,g=b,l,j=e.length,t,y,S,Q="";
				for(l=0;l<j;l++){
					t=e.charAt(l)
					if(t=="'")
						for(y=l+1;y<j;y++){
			...
			function Eb(a,b,e){
				for(var d=0,f;d<a.length;d++){
					f=m(a[d]);f.height(Math.max(0,b-Sa(f,e)))
					}
				}
			function pb(a,b){
			...
			function qb(a){
				a.attr("unselectable","on").css("MozUserSelect","none").bind("selectstart.ui",function(){return false})
				}
			function ab(a){
				a.children().removeClass("fc-first fc-last").filter(":first-child").addClass("fc-first").end().filter(":last-child").addClass("fc-last")
				}
			function rb(a,b){
				a.each(function(e,d){d.className=d.className.replace(/^fc-\w*/,"fc-"+lc[b.getDay()])})
				}
			function Jb(a,b){
				...
				b=[];l&&b.push("background-color:"+l);
				d&&b.push("border-color:"+d);a&&b.push("color:"+a);
				return b.join(";")
				}
			function $a(a,b,e){
				if(m.isFunction(a))
			...
			function mc(a,b){
				...
				if(f("weekMode")=="fixed"){
					ba(S,(6-Q)*7);Q=6}d.title=l(j,f("titleFormat"));
					d.start=j;
					d.end=t;
					d.visStart=y;
					d.visEnd=S;
			...
			function sb(a,b,e){
				...
				function l(w){
					...
					I&&i.each(function(wa,Ga){ea=m(Ga);aa=ca(wa);ea.html(ya(aa,$));rb(ea,aa)});
				...
				function S(w){
					if(!L("selectable")){
						...
						I=ca(I);c("dayClick",this,I,true,w)
					}
					}
				function Q(w,I,R){
					R&&r.build();
				...
				r=new Nb(function(w,I){
					var R,V,ea;i.each(function(aa,va){
						R=m(va);
						V=R.offset().left;
						if(aa)
							ea[1]=V;
			...
			function Rb(a,b,e){
				...
				function g(){
					...
					if(i("allDaySlot")){
						...
						x="<table style='width:100%' class='fc-agenda-allday' cellspacing='0'><tr><th class='"
							...
							+i("allDayText")+
							"</th>" +
							"<td><div class='fc-day-content'><div style='position:relative'/></div></td><th class='"
							+h+
					...
					x="<table class='fc-agenda-slots' style='width:100%' cellspacing='0'><tbody>";//显示的是天周的表格
					ta=zb();
					za=xa(N(ta),bb);
					xa(ta,La);
					for(A=tb=0;ta<za;A++){
						...
						+"</th><td class='"
						+O+"'><div style='position:relative'>&nbsp;</div></td></tr>";
						xa(ta,i("slotMinutes"));
						tb++
						}
					x+="</tbody></table>";
				...
				function l(){
					...
					for(h=0;h<Ba;h++){
						A=ua(h);O=r.eq(h);O.html(s(A,Vb));
						x=M.eq(h);
						+A==+ta
						?x.addClass(Wa+"-state-highlight fc-today"):
							x.removeClass(Wa+"-state-highlight fc-today");
						rb(O.add(x),A)
						}
					}
				function j(h,O){
				...
				function t(h){
					...
					h=w[0].clientWidth;
					if(vb=w.width()-h){
						Va(va,vb);va.show().prev().removeClass("fc-last")
					}else 
						...
						if(A>=0&&A<Ba){
							...
							if(za>ta){
								...
								if(m.isFunction(x)){
									if(h=x(h,O)){
										A.position="absolute";
										A.zIndex=8;wa=m(h).css(A).appendTo(I)
										}
								}else{
									A.isStart=true;
									A.isEnd=true;
			...
			function sc(){
				...
				function g(o,s){
					var v,F=o.length,r,J,M,k,D,Z,ja,ia,la,$="",w,I,R={},V={},ea=pa(),aa;v=z();
					if(w=u("isRTL")){
						I=-1;
						aa=v-1
					}else{
					...
					for(v=0;v<F;v++){
						...
						if(I===false)
							...
							if(I&&I!==true){
								$.remove();
								$=m(I).css({
									position:"absolute",top:r.top,left:r.left}).appendTo(ea)
									}
							r.element=$;
							if(J._id===s)
								t(J,$,r);
					...
					for(v=0;v<F;v++){
						...
						if($=r.element){
							...
							if(r.contentTop!==oa&&R-r.contentTop<10){
								$.find("div.fc-event-time").text(Y(J.start,u("timeFormat"))
										+" - "+J.title);
				...
				function l(o,s){//周和日添加标签框
					...
					+s.top+"px;left:"+
					s.left+"px;"+
					r+"'><div class='fc-event-inner fc-event-skin'"+
					J+"><div class='fc-event-head fc-event-skin'"+
					J+"><div class='fc-event-time'>"+
				...
				function S(o,s,v){
					...
					s.draggable({
						...
						scroll:false,
						grid:[$,w],
						axis:la==1?"y":false,opacity:u("dragOpacity"),
						revertDuration:u("dragRevertDuration"),
						start:function(I,R){
			...
			function Kb(a,b,e){
				...
				function q(G,p){
					p.click(function(L){
						if(!p.hasClass("ui-draggable-dragging")&&!p.hasClass("ui-resizable-resizing"))
							...
							}).hover(function(L){f("eventMouseover",this,G,L)},
									function(L){f("eventMouseout",this,G,L)})
					}
				...
				function ra(G,p,L,c,z,H){
					var T=p._id;
					ha(U[T],L,c);
					f("eventResize",G,p,L,c,function(){ha(U[T],-L,-c);pa(T)}
					,z,H);
					pa(T)
					}
				function sa(G,p,L,c){
			...
			function Qb(){
				...
				function e(i){//生成数据天数据生成
					var C=fa("isRTL"),P,E=i.length,B,n,Y,W;P=ka();
					...
					for(P=0;P<E;P++){
						...
						k+=" class='"
							...
							+r+"px;"
							+M+"'><div class='fc-event-inner fc-event-skin'"
							+(M?" style='"
							+M+"'":"")
							+">";
				...
				function q(i,C,P){
					...
					n.mousedown(function(W){function o(ia){
						...
						if(W.which==1){
							Y=true;
							var s=u.getHoverListener(),v=pa(),
							F=U(),r=E?-1:1,J=E?F-1:0,M=C.css("top"),
							k,D,Z=m.extend({},i),ja=L(i.start);
							K();
				...
				var fa=u.opt,na=u.trigger,
				ga=u.isEventDraggable,
				ra=u.isEventResizable,
				sa=u.eventEnd,
				ha=u.reportEventElement,
				da=u.showEvents,ma=u.hideEvents,
				ua=u.eventResize,pa=u.getRowCnt,
				U=u.getColCnt,ca=u.allDayRow,
				ka=u.allDayBounds,qa=u.colContentLeft,
				G=u.colContentRight,p=u.dayOfWeekCol,
				L=u.dateCell,c=u.compileDaySegs,
				z=u.getDaySegmentContainer,H=u.bindDaySeg,
				T=u.calendar.formatDates,X=u.renderDayOverlay,
				ya=u.clearOverlays,K=u.clearSelection
				}
			function Mb(){
				function a(Q,q,u){
					b();
				...
				g("selectable")&&g("unselectAuto")&&m(document).mousedown(function(Q){
					...
					if(q)if(m(Q.target).parents(q).length)
						return;b(Q)
						})
						}
			function Lb(){
			...
			function Ob(a){
				...
				var e=this,d,f,g,l;
				e.start=function(j,t,y){f=j;g=l=null;
					a.build();
					b(t);
					d=y||"mousemove";
					m(document).bind(d,b)
					...
					columnFormat:{month:"ddd",
								  ...
								  xc={
										...
										buttonText:{
											...
											prevYear:"&nbsp;&gt;&gt;&nbsp;",
											nextYear:"&nbsp;&lt;&lt;&nbsp;"},
										buttonIcons:{prev:"circle-triangle-e",
													 next:"circle-triangle-w"}
											},
								   Aa=m.fullCalendar={version:"1.5.1"},
								   Ja=Aa.views={};
